...
export default function BookVolume({ slot, mobile, reduced, invalidate }) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadTexture(coverUrl(id)), loadTexture(spineUrl(id))])
      .then(async ([cover, spine]) => {
        if (!live) return;
        ...
        setMaps({ cover, spine: sp, foreEdge, headEdge, cloth, clothBump, board, endpaper, headband });
        invalidate();
      })
      .catch(() => {
